...
struct CustomCarDef {
    ...
    // ── Per-wheel overrides (0 / empty = use shared value) ──
    float wheelRadiusRear;           // 0 = use wheelRadius
    float wheelRadiusFront;          // 0 = use wheelRadius
    char  wheelTexRearPath[128];     // empty = use wheelTexPath
    char  wheelTexFrontPath[128];    // empty = use wheelTexPath
    LPDIRECT3DTEXTURE9 pTexWheelRear;
    LPDIRECT3DTEXTURE9 pTexWheelFront;
};
...
// ── Per-wheel overrides (resolved in SetCarParameters; defaults = shared values) ──
extern float g_wheelRadiusRear;                    // visual + physics radius for rear wheel
extern float g_wheelRadiusFront;                   // visual + physics radius for front wheel
extern LPDIRECT3DTEXTURE9 g_pTexWheelResolvedRear;  // resolved rear wheel texture
extern LPDIRECT3DTEXTURE9 g_pTexWheelResolvedFront; // resolved front wheel texture
