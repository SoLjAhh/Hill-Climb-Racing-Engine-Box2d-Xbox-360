...
int            g_totalCarSlots   = 23;  // 22 vehicles + boat; up to 25 custom slots
int            g_totalLevelSlots = 7;  // 7 built-in; up to 25 custom slots
    ...
    if(XInputGetState(0, &state) == ERROR_SUCCESS) {
        ...
        if (g_gameState == STATE_LEVEL_SELECT) {
            ...
            if (currAButton && !g_prevAButton) {
                ...
                if (g_levelSelectIndex == 0)      g_audioManager.PlayMusic("level1");
                else if (g_levelSelectIndex == 1) g_audioManager.PlayMusic("level2");
                else if (g_levelSelectIndex == 2) g_audioManager.PlayMusic("level3");
                else if (g_levelSelectIndex == 3) g_audioManager.PlayMusic("level4");
                else if (g_levelSelectIndex == 4) g_audioManager.PlayMusic("level5");
                else if (g_levelSelectIndex == 5) g_audioManager.PlayMusic("level6");
                else if (g_levelSelectIndex == 6) g_audioManager.PlayMusic("level1"); // level 7 uses level 1 music for now
                else                              g_audioManager.PlayMusic("level1"); // custom levels
        ...
        if (isPlaying) {
            ...
            {
                float fAccelCheck = state.Gamepad.bRightTrigger / 255.0f;
                bool  gasHeld     = (fAccelCheck > 0.1f);

                if (gasHeld && !g_enginePlaying) {
                    // Engine SFX uses PlaySFX with loop=true for simplicity
                    g_audioManager.PlaySFX("engine", 0.6f + fAccelCheck * 0.4f, true);
                    g_enginePlaying = true;
                } else if (!gasHeld && g_enginePlaying) {
                    // Stop engine loop
                    if (g_pEngineVoice) {
                        g_pEngineVoice->Stop(0);
                        g_pEngineVoice = NULL;
                    }
            ...
            if (g_carChassis) {
                ...
                float acceleration = speed - prevSpeed;
                g_hudBoost = acceleration / 20.0f;
