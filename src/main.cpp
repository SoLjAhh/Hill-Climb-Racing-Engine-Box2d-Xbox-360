...
// ── Per-wheel overrides (set by SetCarParameters) ──
float g_wheelRadiusRear;
float g_wheelRadiusFront;
LPDIRECT3DTEXTURE9 g_pTexWheelResolvedRear  = NULL;
LPDIRECT3DTEXTURE9 g_pTexWheelResolvedFront = NULL;
...
HRESULT InitScene() {
    ...
    LoadModTextures();
    LoadRubeSceneIcons();  // load icon PNGs for R.U.B.E. sandbox scenes
...
void Update() {
    ...
    if(XInputGetState(0, &state) == ERROR_SUCCESS) {
        ...
        // Catches every exit path (pause→title, pause→level select, etc.)
        {
            bool playing = (g_gameState >= STATE_PLAYING_LEVEL1 && g_gameState <= STATE_PLAYING_LEVEL7)
                        || g_gameState == STATE_PLAYING_RUBE;
            if (!playing) {
                if (g_enginePlaying) {
        ...
        if (g_gameState == STATE_LEVEL_SELECT) {
            ...
            if (currAButton && !g_prevAButton) {
                // ── R.U.B.E. sandbox scene ──────────────────────────────────────
                // Layout: [0..6] built-in | [7..7+custom-1] custom | [7+custom..] R.U.B.E.
                int rubeBase = 7 + g_numCustomLevels;
                if (g_levelSelectIndex >= rubeBase && g_levelSelectIndex < rubeBase + g_numRubeScenes) {
                    int si = g_levelSelectIndex - rubeBase;
                    // Clean up the normal game world before launching R.U.B.E.
                    if (g_pBox2DWorld != NULL) {
                        delete g_pBox2DWorld;
                        g_pBox2DWorld = NULL;
                        g_bridgeActive = false;
                    ...
                    if (RubeLoadScene(g_rubeScenePaths[si])) {
                        g_gameState = STATE_PLAYING_RUBE;
                        g_audioManager.StopMusic();
                    }
                }
                // ── Normal level (built-in + custom) ────────────────────────────
                else {
                ...
                } // end else (normal level, not R.U.B.E.)
            }
        ...
        if (g_gameState == STATE_PAUSE_MENU) {
            ...
            if (currStartButton && !g_prevStartButton) {
                ...
                if (RubeIsActive()) prevState = STATE_PLAYING_RUBE;
                else if (g_currentLevel == LEVEL_1) prevState = STATE_PLAYING_LEVEL1;
            ...
            if (currAButton && !g_prevAButton) {
                if (g_pauseMenuIndex == 0) {
                    ...
                    if (RubeIsActive()) RubeUnloadScene();
        ...
        // ── R.U.B.E. SANDBOX MODE ─────────────────────────────────────────────
        // Separate physics world driven by rube_loader — camera follows player body,
        // triggers drive the motorised wheel joints, BACK returns to level select.
        if (g_gameState == STATE_PLAYING_RUBE) {
            // Read gamepad triggers (same mapping as car: LEFT=gas, RIGHT=brake/reverse)
            float fGas     = (float)state.Gamepad.bLeftTrigger  / 255.0f;
            float fReverse = (float)state.Gamepad.bRightTrigger / 255.0f;
            float throttle = fGas - fReverse;  // -1 (full reverse) to +1 (full forward)

            // Right stick Y axis → turret/arm control
            float turret = 0.0f;
            short rsY = state.Gamepad.sThumbRY;
            if (rsY > 8000 || rsY < -8000) {  // deadzone
                turret = (float)rsY / 32767.0f;
            }

            float dt = g_Time.fElapsedTime;
            if (dt > 0.0f && dt < 0.05f) {
                RubeUpdate(dt, throttle, turret);
            }

            // Camera follows the player body
            float camX, camY;
            if (RubeGetCameraTarget(camX, camY)) {
                g_fScrollPos = camX;

                // Y tracking — centre the vehicle on screen
                float targetCamY = camY;

                // Snap camera on first frame (g_camY starts at 10.5 from init,
                // far from the R.U.B.E. scene). If more than 5 units away, snap.
                float diff = targetCamY - g_camY;
                if (diff > 5.0f || diff < -5.0f) {
                    g_camY = targetCamY;
                } else {
                    float camLerpRate = 4.5f * dt;
                    if (camLerpRate > 1.0f) camLerpRate = 1.0f;
                    g_camY += diff * camLerpRate;
                }
            }

            // HUD speed display
            g_hudSpeedometer = RubeGetPlayerSpeed() / 25.0f;
            if (g_hudSpeedometer > 1.0f) g_hudSpeedometer = 1.0f;

            // ── Rebuild view/projection matrices ──
            // This MUST happen here because the return below skips the
            // normal camera code at the end of Update().
            {
                float speed     = RubeGetPlayerSpeed();
                float speedNorm = speed / 25.0f;
                if (speedNorm > 1.0f) speedNorm = 1.0f;
                float targetFOV   = (D3DX_PI / 4) * (1.0f + speedNorm * 0.28f);
            ...
            // BACK → restart the R.U.B.E. scene (same as BACK restarts levels)
            if (state.Gamepad.wButtons & XINPUT_GAMEPAD_BACK) {
                RubeRestartScene();
                // Snap camera to new player position immediately
                float rx, ry;
                if (RubeGetCameraTarget(rx, ry)) {
                    g_fScrollPos = rx;
                    g_camY = ry;
                }
                return;
        ...
        if (isPlaying) {
            ...
            if (g_wheelRear && g_wheelFront && g_carChassis) {
                ...
                bool   rearOnGround  = (rearPos.y  - g_wheelRadiusRear)  < (rearTerrainY  + 0.25f);
                bool   frontOnGround = (frontPos.y - g_wheelRadiusFront) < (frontTerrainY + 0.25f);
    ...
    // ── Terrain vertex buffer update (skip in R.U.B.E. mode — no procedural terrain) ──
    if (g_gameState != STATE_PLAYING_RUBE) {
    TEXVERTEX* pV;
    ...
    } // end if (g_gameState != STATE_PLAYING_RUBE)
}
