...
void RenderWheel(float parentX, float parentY, float localX, float localY, float carRot, float spin, float radius, LPDIRECT3DTEXTURE9 wheelTex) {
    ...
    for(int i = 0; i <= CIRCLE_SEGMENTS; i++) {
        ...
        v[i+1].x = radius * cosf(t);
        v[i+1].y = radius * sinf(t);
        v[i+1].z = 0.0f;
...
void RenderPauseMenu() {
    ...
    if (g_carChassis && g_wheelRear && g_wheelFront) {
        ...
        if (!g_isBoatLevel) {
            RenderWheel(rPos.x - g_fScrollPos, rPos.y, 0, 0, cAngle, -wSpin, g_wheelRadiusRear,  g_pTexWheelResolvedRear);
            RenderWheel(fPos.x - g_fScrollPos, fPos.y, 0, 0, cAngle, -wSpin, g_wheelRadiusFront, g_pTexWheelResolvedFront);
        }
    }
...
void RenderLevelSelectScreen() {
    ...
    if (g_levelSelectIndex < 7) {
        ...
        int rubeBase = 7 + g_numCustomLevels;
        if (g_levelSelectIndex >= rubeBase && g_levelSelectIndex < rubeBase + g_numRubeScenes) {
            // R.U.B.E. scene slot — draw a coloured placeholder icon
            isRubeScene = true;
        } else {
            // Custom level slot
    ...
    if (isRubeScene) {
        // Check if a custom icon texture was loaded for this scene
        int si = g_levelSelectIndex - (7 + g_numCustomLevels);
        LPDIRECT3DTEXTURE9 rubeIcon = NULL;
        if (si >= 0 && si < g_numRubeScenes)
            rubeIcon = g_rubeSceneIcons[si];

        if (rubeIcon != NULL) {
            // Use the custom icon PNG
            g_pd3dDevice->SetTexture(0, rubeIcon);
        ...
        // No custom icon — draw clean R.U.B.E. sandbox icon
        // Matches level icon quad size (-0.5 to 0.5, -0.7 to 0.7)
        g_pd3dDevice->SetPixelShader(g_pSolidColourShader);
        g_pd3dDevice->SetTexture(0, NULL);

        D3DXMATRIX matIconWVP;
        matIconWVP = matMenuVP;
        g_pd3dDevice->SetVertexShaderConstantF(0, (float*)&matIconWVP, 4);

        // Dark background fill (same size as level icons)
        float bgCol[4] = { 0.12f, 0.12f, 0.16f, 1.0f };
        g_pd3dDevice->SetPixelShaderConstantF(0, bgCol, 1);
        TEXVERTEX bgQ[4];
        bgQ[0].x = -0.5f; bgQ[0].y =  0.7f; bgQ[0].z = 0; bgQ[0].u = 0; bgQ[0].v = 0;
        bgQ[1].x =  0.5f; bgQ[1].y =  0.7f; bgQ[1].z = 0; bgQ[1].u = 1; bgQ[1].v = 0;
        bgQ[2].x = -0.5f; bgQ[2].y = -0.7f; bgQ[2].z = 0; bgQ[2].u = 0; bgQ[2].v = 1;
        bgQ[3].x =  0.5f; bgQ[3].y = -0.7f; bgQ[3].z = 0; bgQ[3].u = 1; bgQ[3].v = 1;
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, bgQ, sizeof(TEXVERTEX));

        // Border outline (salmon/brown like R.U.B.E. editor)
        float lineCol[4] = { 0.75f, 0.55f, 0.45f, 0.9f };
        g_pd3dDevice->SetPixelShaderConstantF(0, lineCol, 1);
        TEXVERTEX border[5];
        border[0].x = -0.45f; border[0].y =  0.65f; border[0].z = 0; border[0].u = 0; border[0].v = 0;
        border[1].x =  0.45f; border[1].y =  0.65f; border[1].z = 0; border[1].u = 0; border[1].v = 0;
        border[2].x =  0.45f; border[2].y = -0.65f; border[2].z = 0; border[2].u = 0; border[2].v = 0;
        border[3].x = -0.45f; border[3].y = -0.65f; border[3].z = 0; border[3].u = 0; border[3].v = 0;
        border[4] = border[0]; // close
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 4, border, sizeof(TEXVERTEX));

        // Inner box (representing a Box2D body)
        float fillCol[4] = { 0.55f, 0.38f, 0.32f, 0.5f };
        g_pd3dDevice->SetPixelShaderConstantF(0, fillCol, 1);
        TEXVERTEX box[4];
        box[0].x = -0.25f; box[0].y =  0.25f; box[0].z = 0; box[0].u = 0; box[0].v = 0;
        box[1].x =  0.25f; box[1].y =  0.25f; box[1].z = 0; box[1].u = 0; box[1].v = 0;
        box[2].x = -0.25f; box[2].y = -0.25f; box[2].z = 0; box[2].u = 0; box[2].v = 0;
        box[3].x =  0.25f; box[3].y = -0.25f; box[3].z = 0; box[3].u = 0; box[3].v = 0;
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, box, sizeof(TEXVERTEX));

        // Box outline
        g_pd3dDevice->SetPixelShaderConstantF(0, lineCol, 1);
        TEXVERTEX boxLine[5];
        boxLine[0].x = -0.25f; boxLine[0].y =  0.25f; boxLine[0].z = 0; boxLine[0].u = 0; boxLine[0].v = 0;
        boxLine[1].x =  0.25f; boxLine[1].y =  0.25f; boxLine[1].z = 0; boxLine[1].u = 0; boxLine[1].v = 0;
        boxLine[2].x =  0.25f; boxLine[2].y = -0.25f; boxLine[2].z = 0; boxLine[2].u = 0; boxLine[2].v = 0;
        boxLine[3].x = -0.25f; boxLine[3].y = -0.25f; boxLine[3].z = 0; boxLine[3].u = 0; boxLine[3].v = 0;
        boxLine[4] = boxLine[0];
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, 4, boxLine, sizeof(TEXVERTEX));

        // Circle (representing a wheel) — lower right
        const int IC = 16;
        TEXVERTEX cFill[IC + 2];
        float icr = 0.15f, icx = 0.15f, icy = -0.35f;
        cFill[0].x = icx; cFill[0].y = icy; cFill[0].z = 0; cFill[0].u = 0; cFill[0].v = 0;
        for (int si = 0; si <= IC; si++) {
            float a = (float)si / (float)IC * 6.283185f;
            cFill[si+1].x = icx + cosf(a)*icr; cFill[si+1].y = icy + sinf(a)*icr;
            cFill[si+1].z = 0; cFill[si+1].u = 0; cFill[si+1].v = 0;
        }
        g_pd3dDevice->SetPixelShaderConstantF(0, fillCol, 1);
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, IC, cFill, sizeof(TEXVERTEX));
        // Circle outline
        TEXVERTEX cLine[IC + 1];
        for (int si = 0; si <= IC; si++) {
            float a = (float)si / (float)IC * 6.283185f;
            cLine[si].x = icx + cosf(a)*icr; cLine[si].y = icy + sinf(a)*icr;
            cLine[si].z = 0; cLine[si].u = 0; cLine[si].v = 0;
        }
        g_pd3dDevice->SetPixelShaderConstantF(0, lineCol, 1);
        g_pd3dDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, IC, cLine, sizeof(TEXVERTEX));

        // Second circle — lower left
        icx = -0.15f;
        cFill[0].x = icx;
        for (int si = 0; si <= IC; si++) {
            float a = (float)si / (float)IC * 6.283185f;
        ...
        // Restore textured pixel shader
        g_pd3dDevice->SetPixelShader(g_pPixelShader);
        } // end else (no custom icon)
    } // end if (isRubeScene)
    else if (levelIconTex != NULL) {
    ...
    // ── Display scene name for R.U.B.E. levels ──────────────────────────
    // Uses a minimal 3x5 block font rendered with the solid colour shader.
    // No D3DX font dependency — works on Xbox 360 XDK.
    if (isRubeScene) {
        int si = g_levelSelectIndex - (7 + g_numCustomLevels);
        if (si >= 0 && si < g_numRubeScenes && g_rubeSceneNames[si][0]) {
            // 3x5 bitmap font — each char is 15 bits (row0=top, 3 bits per row)
            // bit layout: row0[2,1,0] row1[2,1,0] row2[2,1,0] row3[2,1,0] row4[2,1,0]
            static const unsigned short font3x5[128] = {0};
            static unsigned short fontData[128];
            static bool fontInit = false;
            if (!fontInit) {
                fontInit = true;
                for (int fi = 0; fi < 128; fi++) fontData[fi] = 0;
                // Letters A-Z
                fontData['A'] = 0x2F7E4; // 010 111 101 111 101
                fontData['B'] = 0x7AE97; // 111 010 111 010 111  -> actually let me encode properly
                // Encoding: 15 bits, MSB first = top-left
                // Row 0 bits 14,13,12  Row 1 bits 11,10,9  Row 2 bits 8,7,6  Row 3 bits 5,4,3  Row 4 bits 2,1,0
                fontData['A'] = (2<<12)|(5<<9)|(7<<6)|(5<<3)|5;  // .#. #.# ### #.# #.#
                fontData['B'] = (6<<12)|(5<<9)|(6<<6)|(5<<3)|6;  // ##. #.# ##. #.# ##.
                fontData['C'] = (3<<12)|(4<<9)|(4<<6)|(4<<3)|3;  // .## #.. #.. #.. .##
                fontData['D'] = (6<<12)|(5<<9)|(5<<6)|(5<<3)|6;  // ##. #.# #.# #.# ##.
                fontData['E'] = (7<<12)|(4<<9)|(6<<6)|(4<<3)|7;  // ### #.. ##. #.. ###
                fontData['F'] = (7<<12)|(4<<9)|(6<<6)|(4<<3)|4;  // ### #.. ##. #.. #..
                fontData['G'] = (3<<12)|(4<<9)|(5<<6)|(5<<3)|3;  // .## #.. #.# #.# .##
                fontData['H'] = (5<<12)|(5<<9)|(7<<6)|(5<<3)|5;  // #.# #.# ### #.# #.#
                fontData['I'] = (7<<12)|(2<<9)|(2<<6)|(2<<3)|7;  // ### .#. .#. .#. ###
                fontData['J'] = (1<<12)|(1<<9)|(1<<6)|(5<<3)|2;  // ..# ..# ..# #.# .#.
                fontData['K'] = (5<<12)|(5<<9)|(6<<6)|(5<<3)|5;  // #.# #.# ##. #.# #.#
                fontData['L'] = (4<<12)|(4<<9)|(4<<6)|(4<<3)|7;  // #.. #.. #.. #.. ###
                fontData['M'] = (5<<12)|(7<<9)|(5<<6)|(5<<3)|5;  // #.# ### #.# #.# #.#
                fontData['N'] = (5<<12)|(7<<9)|(7<<6)|(5<<3)|5;  // #.# ### ### #.# #.#
                fontData['O'] = (2<<12)|(5<<9)|(5<<6)|(5<<3)|2;  // .#. #.# #.# #.# .#.
                fontData['P'] = (6<<12)|(5<<9)|(6<<6)|(4<<3)|4;  // ##. #.# ##. #.. #..
                fontData['Q'] = (2<<12)|(5<<9)|(5<<6)|(7<<3)|3;  // .#. #.# #.# ### .##
                fontData['R'] = (6<<12)|(5<<9)|(6<<6)|(5<<3)|5;  // ##. #.# ##. #.# #.#
                fontData['S'] = (3<<12)|(4<<9)|(2<<6)|(1<<3)|6;  // .## #.. .#. ..# ##.
                fontData['T'] = (7<<12)|(2<<9)|(2<<6)|(2<<3)|2;  // ### .#. .#. .#. .#.
                fontData['U'] = (5<<12)|(5<<9)|(5<<6)|(5<<3)|2;  // #.# #.# #.# #.# .#.
                fontData['V'] = (5<<12)|(5<<9)|(5<<6)|(5<<3)|2;  // #.# #.# #.# #.# .#.
                fontData['W'] = (5<<12)|(5<<9)|(5<<6)|(7<<3)|5;  // #.# #.# #.# ### #.#
                fontData['X'] = (5<<12)|(5<<9)|(2<<6)|(5<<3)|5;  // #.# #.# .#. #.# #.#
                fontData['Y'] = (5<<12)|(5<<9)|(2<<6)|(2<<3)|2;  // #.# #.# .#. .#. .#.
                fontData['Z'] = (7<<12)|(1<<9)|(2<<6)|(4<<3)|7;  // ### ..# .#. #.. ###
                // Digits 0-9
                fontData['0'] = (7<<12)|(5<<9)|(5<<6)|(5<<3)|7;  // ### #.# #.# #.# ###
                fontData['1'] = (2<<12)|(6<<9)|(2<<6)|(2<<3)|7;  // .#. ##. .#. .#. ###
                fontData['2'] = (7<<12)|(1<<9)|(7<<6)|(4<<3)|7;  // ### ..# ### #.. ###
                fontData['3'] = (7<<12)|(1<<9)|(7<<6)|(1<<3)|7;  // ### ..# ### ..# ###
                fontData['4'] = (5<<12)|(5<<9)|(7<<6)|(1<<3)|1;  // #.# #.# ### ..# ..#
                fontData['5'] = (7<<12)|(4<<9)|(7<<6)|(1<<3)|7;  // ### #.. ### ..# ###
                fontData['6'] = (7<<12)|(4<<9)|(7<<6)|(5<<3)|7;  // ### #.. ### #.# ###
                fontData['7'] = (7<<12)|(1<<9)|(2<<6)|(2<<3)|2;  // ### ..# .#. .#. .#.
                fontData['8'] = (7<<12)|(5<<9)|(7<<6)|(5<<3)|7;  // ### #.# ### #.# ###
                fontData['9'] = (7<<12)|(5<<9)|(7<<6)|(1<<3)|7;  // ### #.# ### ..# ###
                // Special chars
                fontData['.'] = (0<<12)|(0<<9)|(0<<6)|(0<<3)|2;  // ... ... ... ... .#.
                fontData['-'] = (0<<12)|(0<<9)|(7<<6)|(0<<3)|0;  // ... ... ### ... ...
                fontData['_'] = (0<<12)|(0<<9)|(0<<6)|(0<<3)|7;  // ... ... ... ... ###
                fontData[' '] = 0;
                // Lowercase = uppercase
                for (int li = 'a'; li <= 'z'; li++)
                    fontData[li] = fontData[li - 32];
            }

            g_pd3dDevice->SetPixelShader(g_pSolidColourShader);
            g_pd3dDevice->SetTexture(0, NULL);

            const char* name = g_rubeSceneNames[si];
            int nameLen = 0;
            while (name[nameLen]) nameLen++;

            // Character size in world units and starting position
            float charW = 0.08f;   // width of one pixel column
            float charH = 0.08f;   // height of one pixel row
            float gap   = 0.04f;   // gap between characters
            float totalW = nameLen * (charW * 3 + gap) - gap;
            float startX = -totalW * 0.5f;
            float startY = -1.2f;  // below the icon

            D3DXMATRIX matTextWVP;
            matTextWVP = matMenuVP;
            g_pd3dDevice->SetVertexShaderConstantF(0, (float*)&matTextWVP, 4);

            float textCol[4] = { 0.78f, 0.60f, 0.45f, 1.0f };
            g_pd3dDevice->SetPixelShaderConstantF(0, textCol, 1);

            for (int ci = 0; ci < nameLen && ci < 32; ci++) {
                unsigned char ch = (unsigned char)name[ci];
                if (ch > 127) ch = '?';
                unsigned short bits = fontData[ch];

                float cx = startX + ci * (charW * 3 + gap);

                for (int row = 0; row < 5; row++) {
                    for (int col = 0; col < 3; col++) {
                        int bitIdx = (4 - row) * 3 + (2 - col);
                        if (bits & (1 << bitIdx)) {
                            float px = cx + col * charW;
                            float py = startY - row * charH;
                            TEXVERTEX pq[4];
                            pq[0].x = px;        pq[0].y = py;        pq[0].z = 0; pq[0].u = 0; pq[0].v = 0;
                            pq[1].x = px+charW;  pq[1].y = py;        pq[1].z = 0; pq[1].u = 0; pq[1].v = 0;
                            pq[2].x = px;        pq[2].y = py-charH;  pq[2].z = 0; pq[2].u = 0; pq[2].v = 0;
                            pq[3].x = px+charW;  pq[3].y = py-charH;  pq[3].z = 0; pq[3].u = 0; pq[3].v = 0;
                            g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, pq, sizeof(TEXVERTEX));
                        }
                    }
                }
            }
...
void Render() {
    ...
    // ── R.U.B.E. SANDBOX — draw Box2D polygon shapes, skip normal terrain/car ──
    if (g_gameState == STATE_PLAYING_RUBE) {
        g_pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER,
                            D3DCOLOR_XRGB(12, 12, 28), 1.0f, 0);
        g_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
        g_pd3dDevice->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
        g_pd3dDevice->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
        g_pd3dDevice->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA);
        // Disable depth test/write — all R.U.B.E. geometry is 2D.
        // Draw order in RubeRenderScene controls layering instead.
        g_pd3dDevice->SetRenderState(D3DRS_ZENABLE, FALSE);
        ...
        // Restore depth state for other rendering
        g_pd3dDevice->SetRenderState(D3DRS_ZENABLE, TRUE);
    ...
    if (g_carChassis && g_wheelRear && g_wheelFront) {
        ...
        if (g_isBoatLevel) {
            ...
            RenderWheel(rearPos.x  - g_fScrollPos, rearPos.y,  0, 0, chassisAngle, -wheelSpin, g_wheelRadiusRear,  g_pTexWheelResolvedRear);
            RenderWheel(frontPos.x - g_fScrollPos, frontPos.y, 0, 0, chassisAngle, -wheelSpin, g_wheelRadiusFront, g_pTexWheelResolvedFront);
        }
    }
