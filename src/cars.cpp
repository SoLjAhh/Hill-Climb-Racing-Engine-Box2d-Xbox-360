...
void SetCarParameters(CarType car) {
    // ── Reset per-wheel overrides (sentinel = not yet set by cfg) ──
    g_wheelRadiusRear        = -1.0f;
    g_wheelRadiusFront       = -1.0f;
    g_pTexWheelResolvedRear  = NULL;
    g_pTexWheelResolvedFront = NULL;
    ...
    //============================================================================================
    // ✅ PER-WHEEL RESOLUTION — applies after all car params + cfg overrides are set
    // Defaults: both wheels use shared g_visualWheelRadius and GetTireTexture().
    // LoadCarCfg may have set g_wheelRadiusRear / g_wheelRadiusFront to override values.
    // Custom cars use per-wheel fields from CustomCarDef if they were configured.
    //============================================================================================

    // ── Radius: if LoadCarCfg didn't set per-wheel overrides, use shared radius ──
    if (g_wheelRadiusRear  <= 0.0f) g_wheelRadiusRear  = g_visualWheelRadius;
    if (g_wheelRadiusFront <= 0.0f) g_wheelRadiusFront = g_visualWheelRadius;

    // ── Custom car per-wheel radius (settings.cfg overrides) ──
    int customIdx = (int)car - CAR_CUSTOM_BASE;
    if (customIdx >= 0 && customIdx < g_numCustomCars && g_customCars[customIdx].loaded) {
        const CustomCarDef& cc = g_customCars[customIdx];
        if (cc.wheelRadiusRear  > 0.0f) g_wheelRadiusRear  = cc.wheelRadiusRear;
        if (cc.wheelRadiusFront > 0.0f) g_wheelRadiusFront = cc.wheelRadiusFront;
    }

    // ── Textures: default both wheels to the car's standard tire texture ──
    // Only set defaults if LoadCarCfg didn't already load per-wheel textures
    LPDIRECT3DTEXTURE9 baseTex = GetTireTexture(car);
    if (g_pTexWheelResolvedRear  == NULL) g_pTexWheelResolvedRear  = baseTex;
    if (g_pTexWheelResolvedFront == NULL) g_pTexWheelResolvedFront = baseTex;

    // ── Custom car per-wheel textures (loaded from settings.cfg paths) ──
    if (customIdx >= 0 && customIdx < g_numCustomCars && g_customCars[customIdx].loaded) {
        const CustomCarDef& cc = g_customCars[customIdx];
        if (cc.pTexWheelRear  != NULL) g_pTexWheelResolvedRear  = cc.pTexWheelRear;
        if (cc.pTexWheelFront != NULL) g_pTexWheelResolvedFront = cc.pTexWheelFront;
    }

    // ── Built-in car per-wheel textures (loaded by LoadCarCfg into override globals) ──
    // LoadCarCfg sets these directly if wheel_tex_rear / wheel_tex_front keys exist.
    // They remain NULL if no per-wheel texture was configured.
}
