...
void LoadConfig() {
    ...
    for (int i = 0; i < MAX_CUSTOM_CARS;   i++) {
        ...
        memset(&g_customCarSfx[i], 0, sizeof(CustomCarSfx));
        ...
        g_customCars[i].wheelRadiusRear   = 0.0f;  // 0 = use shared wheelRadius
        g_customCars[i].wheelRadiusFront  = 0.0f;  // 0 = use shared wheelRadius
    }
    ...
    memset(g_customLevelMusic, 0, sizeof(g_customLevelMusic));
    ...
    do { \
        ...
        *eq = '\0'; \
        ...
        if (section == CFG_GLOBAL) { \
            ...
            else if (strcmp(line,"wheel_radius_rear")    == 0) cc.wheelRadiusRear   = fval; \
            else if (strcmp(line,"wheel_radius_front")   == 0) cc.wheelRadiusFront  = fval; \
            else if (strcmp(line,"wheel_tex_rear")       == 0) CfgStrCopy(cc.wheelTexRearPath,  128, valStr); \
            else if (strcmp(line,"wheel_tex_front")      == 0) CfgStrCopy(cc.wheelTexFrontPath, 128, valStr); \
            else if (strcmp(line,"sfx_engine")  == 0 && carIdx >= 0) CfgStrCopy(g_customCarSfx[carIdx].engine,  128, valStr); \
            else if (strcmp(line,"sfx_idle")    == 0 && carIdx >= 0) CfgStrCopy(g_customCarSfx[carIdx].idle,    128, valStr); \
            else if (strcmp(line,"sfx_reverse") == 0 && carIdx >= 0) CfgStrCopy(g_customCarSfx[carIdx].reverse, 128, valStr); \
            else if (strcmp(line,"sfx_horn")    == 0 && carIdx >= 0) CfgStrCopy(g_customCarSfx[carIdx].horn,    128, valStr); \
            ...
            else if (strcmp(line,"music")            == 0 && lvlIdx >= 0) CfgStrCopy(g_customLevelMusic[lvlIdx], 128, valStr); \
        } \
    } while(0)
...
void LoadModTextures() {
    for (int i = 0; i < g_numCustomCars; i++) {
        ...
        // ── Per-wheel textures (optional overrides) ──
        if (cc.wheelTexRearPath[0])
            D3DXCreateTextureFromFileA(g_pd3dDevice, cc.wheelTexRearPath,  &cc.pTexWheelRear);
        if (cc.wheelTexFrontPath[0])
            D3DXCreateTextureFromFileA(g_pd3dDevice, cc.wheelTexFrontPath, &cc.pTexWheelFront);
    }
...
bool LoadCarCfg(const char* carName) {
    ...
    while (line) {
        ...
        *eq = '\0';
        ...
        // ── Per-wheel radius overrides ──
        else if (strcmp(key, "wheel_radius_rear")   == 0) g_wheelRadiusRear       = fval;
        else if (strcmp(key, "wheel_radius_front")  == 0) g_wheelRadiusFront      = fval;
        // ── Per-wheel texture overrides (loaded immediately — D3D device exists) ──
        else if (strcmp(key, "wheel_tex_rear") == 0) {
            D3DXCreateTextureFromFileA(g_pd3dDevice, val, &g_pTexWheelResolvedRear);
        }
        else if (strcmp(key, "wheel_tex_front") == 0) {
            D3DXCreateTextureFromFileA(g_pd3dDevice, val, &g_pTexWheelResolvedFront);
        }
