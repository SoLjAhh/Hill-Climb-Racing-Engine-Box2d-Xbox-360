...
void InitBox2DPhysics() {
    ...
    b2CircleShape wheelShapeRear;
    wheelShapeRear.m_radius = g_wheelRadiusRear;
    
    b2FixtureDef wheelFixtureRear;
    wheelFixtureRear.shape       = &wheelShapeRear;
    wheelFixtureRear.density     = 1.0f;
    wheelFixtureRear.friction    = g_isAirplane ? 0.0f : 1.9f;
    wheelFixtureRear.restitution = g_isAirplane ? 0.0f : 0.2f;
    wheelFixtureRear.isSensor    = g_isAirplane;
    ...
    g_wheelRear->CreateFixture(&wheelFixtureRear);
    g_wheelRear->SetBullet(true);
    
    b2CircleShape wheelShapeFront;
    wheelShapeFront.m_radius = g_wheelRadiusFront;

    b2FixtureDef wheelFixtureFront;
    wheelFixtureFront.shape       = &wheelShapeFront;
    wheelFixtureFront.density     = 1.0f;
    wheelFixtureFront.friction    = g_isAirplane ? 0.0f : 1.9f;
    wheelFixtureFront.restitution = g_isAirplane ? 0.0f : 0.2f;
    wheelFixtureFront.isSensor    = g_isAirplane;
    ...
    g_wheelFront->CreateFixture(&wheelFixtureFront);
...
void LoadLevel(Level level) {
    ...
    // ── JSON level overrides (jumps, water, gravity, spawn) ─────────────
    bool jsonLevel = false;
    float jsonSpawnX = CAR_START_X;
    float jsonSpawnY = CAR_START_Y;
    if ((int)level >= LEVEL_CUSTOM_1) {
        int ci = (int)level - (int)LEVEL_CUSTOM_1;
        if (ApplyJsonLevelData(ci)) {
            jsonLevel = true;
            jsonSpawnX = g_jsonLevelExtra[ci].spawnX;
            jsonSpawnY = g_jsonLevelExtra[ci].spawnY;
        }
    }
